...
// Read a file
function readOBJFileAdd(fileName, gl, model, scale, reverse) {
  console.log("readOBJFileAdd");
  var request = new XMLHttpRequest();

  request.onreadystatechange = function() {
    if (request.readyState === 4 && request.status !== 404) {
      onReadOBJFileAdd(request.responseText, fileName, gl, model, scale, reverse);
    }
  }
  request.open('GET', fileName, true); // Create a request to acquire the file
  request.send();                      // Send the request
...
var g_objDocList = new Array();
// OBJ File has been read
function onReadOBJFileAdd(fileString, fileName, gl, o, scale, reverse) {
  console.log("onReadOBJFileAdd");
  var objDoc = new OBJDoc(fileName);  // Create a OBJDoc object
  var result = objDoc.parse(fileString, scale, reverse); // Parse the file
  if (!result) {
    g_objDoc = null; g_drawingInfo = null;
...
// OBJ File has been read compreatly
function onReadCompleteAdd(gl, model, objDocList) {
  console.log("onReadCompleteAdd");
  // Acquire the vertex coordinates and colors from OBJ file
  var drawingInfo = objDocList[0].getDrawingInfo();
  drawingInfoVertList = new Array();
  drawingInfoVertList2 = new Array();
  drawingInfoNormalsList = new Array();
  drawingInfoColorsList = new Array();
  drawingInfoIndicesList = new Array();
  for(var i = 0; i < objDocList.length; i++)
  {
	  var partialInfo = g_objDocList[i].getDrawingInfo();
	  drawingInfoVertList.push(partialInfo.vertices);
	  console.log(partialInfo.vertices);
	  console.log(drawingInfoVertList2.concat(partialInfo.vertices));
	  drawingInfoVertList2 = drawingInfoVertList2.concat(partialInfo.vertices);
	  drawingInfoNormalsList.push(partialInfo.normals);
	  drawingInfoColorsList.push(partialInfo.colors);
	  drawingInfoIndicesList.push(partialInfo.indices);
  }
  console.log(drawingInfoVertList);
  console.log(drawingInfoVertList.flat())
  console.log(new Float32Array(drawingInfoVertList.flat(1)));
  console.log(drawingInfoVertList2);
  // Write date into the buffer object
  gl.bindBuffer(gl.ARRAY_BUFFER, model.vertexBuffer);
  gl.bufferData(gl.ARRAY_BUFFER, drawingInfo.vertices, gl.STATIC_DRAW);
...
// Parsing the OBJ file
OBJDoc.prototype.parseAdd = function(fileString, scale, reverse) {
  var lines = fileString.split('\n');  // Break up into lines and store them as array
  lines.push(null); // Append null
  var index = 0;    // Initialize index of line
...
class Camera{
	constructor(_position, _orientation, _nearPlane, _farPlane, _fovAngle)
	{
		this.position =  _position;
		this.orientation = _orientation;
		this.nearPlane = _nearPlane;
		this.farPlane = _farPlane;
		this.fovAngle = _fovAngle;
	}
	
	GetLookAtMatrix(targetx, targety, targetz)
	{
		var M = new Mat4();
		M.SetIdentity();
